...
router.post('/generate', auth_1.authenticateToken, checkLicense_1.checkLicense, async (req, res) => {
    try {
        const userId = req.user.id;
        const { documentIds, additionalFacts, documents: clientDocuments } = req.body;
        ...
        const report = await aiService_1.aiService.generateReport(prompt);
        res.json({
            report: report,
 ...
 */
function buildComparisonPrompt(documents, additionalFacts) {
