...
router.get('/:id/stream', auth_1.authenticateToken, checkLicense_1.checkLicense, async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
        const doc = await models_1.PolicyDocumentModel.findOne({ where: { id, userId } });
        ...
        const fileBuffer = await fileStorage_1.FileStorageService.getFileBuffer(doc.fileKey);
